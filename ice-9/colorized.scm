...
(define (generate-colored-prompt repl)
  (let ((level (length (cond
        ...
        (rl (using-readline?)))
    (string-append
     (colorize-string (object->string (language-name (repl-language repl))) '(MAGENTA) rl)
     (colorize-string "@(" '(CYAN) rl)
     (colorize-string (format #f "~{~a~^ ~}" (module-name (current-module))) '(WHITE) rl)
     (colorize-string ")" '(CYAN) rl)
     (colorize-string (if (zero? level) "" (format #f " [~a]" level)) '(RED) rl)
     (colorize-string "> " '(CYAN) rl))))
